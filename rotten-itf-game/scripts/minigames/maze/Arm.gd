...
func fade_out():
	...
	tween.tween_property(base_node, "modulate", Color(1,1,1,0), 2.0)
...
func _physics_process(delta: float) -> void:
	...
	if reduce:
		var shrink_factor := pow(0.8, delta)  # shrink to some% per second
		for i in range(_segment_lengths.size()):
			_segment_lengths[i] *= shrink_factor
			if _segment_lengths[i] < 1.5:
